...
lemma run_loop_visits_all (inst : WeightedGraph V) (state : DijkstraState V) :
  ...
  -- if all visited, then we do nothing and done
  | zero =>
    unfold run_loop
  ...
  -- now for n+1 case, unfold to go to n case
  | succ n ih =>
    unfold run_loop
    ...
    -- insert u increase cardinality, so transi to n case
...
lemma init_invariant (inst : WeightedGraph V) :
  ...
  · -- unvisited v has distances v = ⨅ u ∈ visited, distances u + weight u v
    ...
    simp [Finset.mem_singleton] at hv
    simp [hv]

/-
# 4. The Logical Bridge
Formalizing the invariant and the lemmas for the final theorem.
-/

-- Greedy choice picks the correct distance (Contradiction proof).
...
by
  ...
  -- Use the invariant clause for this unvisited u:
  ...
  --"shortest_path_dist u" is a lower bound of that infimum
  have h_le : inst.shortest_path_dist u ≤ state.distances u := by
    sorry

  -- Show `state.distances u ≤ inst.shortest_path_dist u`.
  -- using “first unvisited vertex on w” argument
  -- follows from "relaxation cannot undershoot true distance".
...
-- Relaxation step maintains the restricted infimum for the frontier.
