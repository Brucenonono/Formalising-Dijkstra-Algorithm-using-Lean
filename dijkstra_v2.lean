...
lemma choose_next_min
    (inst : WeightedGraph V) (state : DijkstraState V)
    (hif : state.visited.card < Fintype.card V) :
    ∀ v ∈ (Finset.univ \ state.visited),
      state.distances (inst.choose_next state) ≤ state.distances v := by sorry
-- Updates distances for unvisited neighbors of u.
...
decreasing_by
  simp_wf
  rw [Finset.card_insert_of_notMem (h_unvisited h)]
  omega

def initial_state (inst : WeightedGraph V) : DijkstraState V := {
  visited := {inst.source},
  distances := (λ v => if v = inst.source then 0 else ⊤)
}
...
-- Cleaner version using ordinary induction
lemma run_loop_preserves_invariant (inst : WeightedGraph V) (state : DijkstraState V) :
  ...
  induction hn :(Fintype.card V - state.visited.card) using Nat.rec
  ...
  | succ n ih =>
    ...
    have h_u_not_mem : inst.choose_next state ∉ state.visited :=
      h_unvisited hif

    have h' : (insert (inst.choose_next state) state.visited).card = state.visited.card + 1 :=
      Finset.card_insert_of_notMem (h_unvisited hif)
